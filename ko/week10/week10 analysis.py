...
- 충격력-시간 그래프 (동적 줌인)
- AI 파손 확률 예측 바 차트
위 6가지 요소를 하나의 대시보드에 통합 구현하였습니다.
"""
...
# ==========================================
# 3. 마스터 GUI 화면 구성 (2x4 GridSpec)
# ==========================================
fig = plt.figure(figsize=(19, 9))
fig.subplots_adjust(bottom=0.20, top=0.90, left=0.04, right=0.96, wspace=0.3, hspace=0.4)
fig.suptitle('[Step 5 마스터] 낙하 궤적 통합 시뮬레이터 (위치/속도/가속도/충격력 + AI)', fontsize=18, fontweight='bold')
...
ax_anim.set_title('물리 낙하 시뮬레이션', fontsize=12)
...
ax_pos.set_title('위치-시간 (Position-Time)', fontsize=12)
...
# [차트 3: 속도-시간] (Top-2) - 새로 추가됨
ax_vel = fig.add_subplot(gs[0, 2])
ax_vel.set_xlim(0, t_total)
ax_vel.set_ylim(-8, 8)
ax_vel.set_title('속도-시간 (Velocity-Time)', fontsize=12)
ax_vel.set_ylabel('속도 (m/s)')
ax_vel.grid(True, linestyle='--', alpha=0.6)
line_v_h, = ax_vel.plot([], [], 'r-', lw=2, label='맨바닥')
line_v_e, = ax_vel.plot([], [], color='orange', lw=2, label='스티로폼')
line_v_b, = ax_vel.plot([], [], 'b-', lw=2, label='에어캡')
line_v_t, = ax_vel.plot([], [], 'g-', lw=2, label='트레이')

# [차트 4: 가속도-시간] (Top-3)
ax_accel = fig.add_subplot(gs[0, 3])
ax_accel.set_xlim(0, t_total)
ax_accel.set_ylim(-20, 4000)
ax_accel.set_title('가속도-시간 (Acceleration-Time)', fontsize=12)
...
ax_ai.set_title('AI 손상 확률 예측', fontsize=12)
...
# [차트 6: 충격력-시간 확대] (Bottom-1~3)
ax_force = fig.add_subplot(gs[1, 1:])
ax_force.set_ylim(0, 900)
ax_force.set_title('물리 충격력 (Newton) - 임팩트 순간 동적 확대(Zoom)', fontsize=12)
...
def update_sim(val=None):
    ...
    y_e, f_e, a_e, v_e = calc_traj(m, h, pack_props[1][0], pack_props[1][1])
    y_b, f_b, a_b, v_b = calc_traj(m, h, pack_props[2][0], pack_props[2][1])
    y_t, f_t, a_t, v_t = calc_traj(m, h, pack_props[3][0], pack_props[3][1])
...
def update_anim(f):
    ...
    # 속도-시간
    line_v_h.set_data(time_array[:f+1], global_data['v'][0][:f+1])
    line_v_e.set_data(time_array[:f+1], global_data['v'][1][:f+1])
    line_v_b.set_data(time_array[:f+1], global_data['v'][2][:f+1])
    line_v_t.set_data(time_array[:f+1], global_data['v'][3][:f+1])
    ...
    return apple_h, apple_e, apple_b, apple_t, line_p_h, line_p_e, line_p_b, line_p_t, line_v_h, line_v_e, line_v_b, line_v_t, line_a_h, line_a_e, line_a_b, line_a_t, line_f_h, line_f_e, line_f_b, line_f_t
